...
def connectionPostgres():
    ...
    connection = psycopg2.connect(user=Config.getInnested(
                                      "database", "user_db"),
                                  password=Config.getInnested(
                                      "database", "password_db"),
                                  host=Config.getInnested("database", "host"),
                                  port=Config.getInnested("database", "port"),
                                  database=Config.getInnested(
                                  "database", "db_name"))
...
def connectionMysql():
    connection = mysql.connector.connect(
                                        user=Config.getInnested(
                                            "database", "user_db").__str__(),
                                        password=Config.get(
                                         "database", "password_db").__str__(),
                                        host=Config.get(
                                             "database", "host").__str__(),
                                        database=Config.get(
                                             "database", "db_name").__str__()
                                        )
    return connection
